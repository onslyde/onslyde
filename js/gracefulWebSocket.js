   ...
   $.extend({
      ...
      gracefulWebSocket: function (url, options) {
         ...
         this.defaults = {
            ...
            fallbackSendURL: url.replace('ws:', 'http:').replace('wss:', 'https:').replace('8081','8080'),
            fallbackSendMethod: 'POST',
            fallbackPollURL: url.replace('ws:', 'http:').replace('wss:', 'https:').replace('8081','8080'),
         ...
         function FallbackSocket() {
            ...
            var pollInterval;
            var openTimout;
            var posturl = opts.fallbackSendURL + '/go/attendees/vote?' + $.param( opts.fallbackPollParams );
